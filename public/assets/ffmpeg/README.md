...
该版本是单线程版本，没有使用ShareArrayBuffer的方案，可以支持跨域
...
:::caution If you are a [vite](https://vitejs.dev/) user, use `esm` in **baseURL** instead of `umd`:
...
:::caution As SharedArrayBuffer is required for multithread version, make sure you have have fulfilled
...
:::danger `progress` is an experimental feature and might not work for many cases (ex. concat video files, convert image
files, ...). Please use with caution. :::
